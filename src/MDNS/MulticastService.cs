...
///   </para>
/// </remarks>
public sealed class MulticastService : IResolver, IDisposable
{
    ...
    private List<NetworkInterface> _knownNics = new();
    ...
    ///   will be ignored.
    /// </remarks>
    public bool IgnoreDuplicateMessages { get; }
    ...
    /// <summary>
    ///   Get the network interfaces that are usable.
    /// </summary>
    /// <returns>
    ...
    private void FindNetworkInterfaces()
    {
        try
        {
            ...
            foreach (NetworkInterface? nic in _knownNics.Where(k => !currentNics.Any(n => k.Id == n.Id)))
            {
                oldNics.Add(nic);
            }

            foreach (NetworkInterface? nic in currentNics.Where(nic => !_knownNics.Any(k => k.Id == nic.Id)))
            {
                newNics.Add(nic);
            }
    ...
    public void SendAnswer(Message answer, MessageEventArgs query, bool checkDuplicate = true)
    {
        ...
        {
            r.TTL = (r.TTL > _maxLegacyUnicastTtl) ? _maxLegacyUnicastTtl : r.TTL;
        }
    ...
    private void Send(DnsObject msg, bool checkDuplicate, IPEndPoint? remoteEndPoint = null)
    {
